    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th colspan="{{ count($selectedItems) }}">Items (Destrezas)</th>
            ...
            @foreach($studentsData as $index => $student)
                <tr>
                    ...
                    @foreach($selectedItems as $item)
                        <td>{{ isset($student->selections[$item->id]) && $student->selections[$item->id] ? '1' : '' }}</td>
                    @endforeach
        ...
        <tfoot>
            ...
            <tr style="font-weight: bold; background: #ffffff;">
                ...
                @foreach($selectedItems as $item)
                    <td>{{ number_format($columnPercentages[$item->id] ?? 0, 2, ',', '') }}%</td>
                @endforeach
            ...
            <tr style="background: #ffffff; text-align: center;">
                ...
                @foreach($selectedItems as $item)
                    <td style="font-size: 5pt; padding-top: 5px; padding-bottom: 5px; vertical-align: top;">
                        {!! implode('<br>', str_split(str_replace('EN PROCESO', 'PROCESO', $columnEscalas[$item->id] ?? ''))) !!}
                    </td>
                @endforeach
                <td style="border: none;"></td>
                <td style="border: none;"></td>
